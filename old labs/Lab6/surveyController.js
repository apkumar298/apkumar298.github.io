...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        var explain = readData("explain");
        var likert = readData("likert");
        var interest = readData("interest");
        var aspect = readData("aspect");
        var general = readData("general");
        var comment = readData("comment");
        var color = readData("color");
        var fruit = readData("fruit");
        var animal = readData("animal");
        res.render('showResults', {results: [color, fruit, animal, explain, likert, interest, aspect, general, comment]});
        console.log([color, fruit, animal, explain, likert, interest, aspect, general, comment]);
    });
    ...
    app.post('/niceSurvey', urlencodedParser, function(req, res){
        ...
        for (var key in json){
            ...
            // in the case of checkboxes, the user might check more than one
            if ((key === "aspect") && (json[key].length === 2)){
                for (var item in json[key]){
                    combineCounts(key, json[key][item]);
                }
            }
